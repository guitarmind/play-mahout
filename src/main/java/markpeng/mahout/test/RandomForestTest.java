...
/**
 * https://github.com/jennifersmith/machinenursery/blob/master/src/main/java/
 * MahoutPlaybox.java
 * 
 * @author markpeng
 *
...
public class RandomForestTest {
	private static DecisionForest buildForest(int numberOfTrees, Data data) {
		...
		SequentialBuilder forestBuilder = new SequentialBuilder(
				RandomUtils.getRandom(), treeBuilder, data.clone());
		treeBuilder.setM(m);
	...
	public static void main(String[] args) throws IOException,
		...
		if (args.length < 3) {
			throw new RuntimeException(
					"Usage: main.java.MahoutPlaybox [numberOfTrees] [ignorePixels(true|false)] [useThresholding(true|false)]");
		}
		...
		System.out.println("Building " + numberOfTrees
				+ " trees. Ignoring pixels? " + ignorePixels
				+ ". Using thresholding? " + useThresholding);
		if (ignorePixels) {
		...
		// testFileAsStringArray("data/huge-test.csv");
		...
		double duration = new BigDecimal(endTime - startTime).divide(
				new BigDecimal("1000")).doubleValue();
	...
	private static void saveTree(int numberOfTrees, DecisionForest forest)
			throws IOException {
		DataOutputStream dos = new DataOutputStream(new FileOutputStream(
				"saved-trees/" + numberOfTrees + "-trees-"
						+ System.currentTimeMillis() + "-"
						+ RandomUtils.getRandom().nextInt() + ".txt"));
		forest.write(dos);
	}

	public static void runIteration(int numberOfTrees,
			String[] trainDataValues, String[] testDataValues, String descriptor)
		...
		// List<org.apache.mahout.classifier.df.node.Node> trees = new
		// ArrayList<Node>();
		//
		...
		// filesUsed++;
		// try {
		// MultiDecisionForest forest = MultiDecisionForest.load(new
		// Configuration(), new Path(file.getPath()));
		...
		try {
			...
			for (int i = 0; i < test.size(); i++) {
				...
				int actualLabel = data.getDataset().valueOf(0,
						String.valueOf((int) actualIndex));
			...
			System.out.println("Number of trees: " + numberOfTrees
					+ " -> Number correct: " + numberCorrect + " of "
					+ numberOfValues + " (" + percentageCorrect + ")");
			out.close();
		} catch (Exception e) {
			e.printStackTrace();
	...
	private static Data loadData(String[] sData, String descriptor)
