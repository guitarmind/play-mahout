...
public class EntityFeatureWithTFGenerator {
		...
		try {
			...
			while ((aLine = trainIn.readLine()) != null) {
				...
				for (String s : finalEntities) {
					if (!s.contains(" ")) {
						if (htokens.containsKey(s)) {
							rowResult[sIndex] = rowResult[sIndex]
									+ htokens.get(s);
							System.out
									.println(s + " ==> entity: " + postHeader);
						}
						if (abtokens.containsKey(s)) {
							rowResult[sIndex] = rowResult[sIndex]
									+ abtokens.get(s);
							System.out.println(s + " ==> entity: " + postAbst);
						}
					} else {
						if (postHeader.contains(s)) {
							rowResult[sIndex]++;
							System.out
									.println(s + " ==> entity: " + postHeader);
						}
						if (postAbst.contains(s)) {
							rowResult[sIndex]++;
							System.out.println(s + " ==> entity: " + postAbst);
						}
					}
				...
				for (String t : topicWords) {
					if (htokens.containsKey(t)) {
						rowResult[tIndex] = rowResult[tIndex] + htokens.get(t);
						System.out.println(t + " ==> topic: " + postHeader);
					}
					if (abtokens.containsKey(t)) {
						rowResult[tIndex] = rowResult[tIndex] + abtokens.get(t);
						System.out.println(t + " ==> topic: " + postAbst);
					}
				...
				for (Integer v : rowResult) {
					if (vIndex < rowResult.length - 1)
						resultStr.append(df.format(Math.log(1 + v)) + ",");
					else
						resultStr.append(df.format(Math.log(1 + v)) + newLine);
					vIndex++;
				}
	...
	public List<String> topicWordExtraction(String trainFile, String testFile,
			List<String> finalEntities) throws Exception {
	...
	public static void main(String[] args) throws Exception {
		...
		args[3] = "/home/markpeng/Share/Kaggle/The Analytics Edge Competition/entityTF_Train.csv";
		args[4] = "/home/markpeng/Share/Kaggle/The Analytics Edge Competition/entityTF_Test.csv";
		...
		EntityFeatureWithTFGenerator worker = new EntityFeatureWithTFGenerator();
