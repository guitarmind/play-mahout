...
public class PredictResultReader {
	public static Map<String, Integer> readCategoryByTweetIds(
			Configuration configuration, String tweetFileName) throws Exception {
		Map<String, Integer> categoryByTweetIds = new HashMap<String, Integer>();
		BufferedReader reader = new BufferedReader(
				new FileReader(tweetFileName));
		while (true) {
	...
	public static void main(String[] args) throws Exception {
		if (args.length < 3) {
			System.out
					.println("Arguments: [tweet file] [label index] [class labels]");
			return;
		}
		...
		Map<String, Integer> categoryByTweetIds = readCategoryByTweetIds(
				configuration, tweetCategoryIdsPath);
